...
repository that already uses tuhdoo. There is no server to register with and
no account to create: the coordination ledger is an orphan git branch
(`tuhdoo` by default) living inside the repo you are about to clone, and
...
A plain, full clone (as above) is the recommended shape. Other shapes, for
the record:

- **`--single-branch` works.** The data branch never needs to be part of
  your clone: on first start, tuhdoo fetches `refs/heads/tuhdoo` from the
  remote by an explicit refspec (into its own `refs/tuhdoo/remote` tracking
  ref), so no remote-tracking configuration is assumed or consulted.
- **Shallow clones (`--depth=…`) are fine.** `--depth` implies
  `--single-branch`, so the data branch is fetched fresh from the remote as
  above — and tuhdoo replays state from the data branch's tip tree only, so
  truncated history on your code branches never matters to it.
- **Do not run tuhdoo from a fork.** The daemon syncs with the `origin`
  remote and nothing else. Cloned from a fork, it will faithfully sync the
  *fork's* data branch — silently maintaining a divergent copy of the
  team's ledger that the shared repo never sees. Clone the repository the
  team actually shares.
...
then invoke it as `npx tuhdoo` everywhere this doc says `tuhdoo`.

Via a release archive: download the archive for your platform and the
...
remote already carries a `tuhdoo` branch (it does; that is the ledger you
are joining), the daemon adopts that branch as its local copy instead of
minting a fresh one. Nothing to configure, no flags.
...
- **Exempt the data branch from pull-request and review requirements.**
  If your host enforces rules on all branches (rulesets, protected
  branches, or whatever your host calls them — e.g. a GitHub ruleset
