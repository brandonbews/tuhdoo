...
## T5. MCP verb surface (v0) *(revised 2026-07-30 — out-of-band answers: `relay_answer` is the eleventh verb; revised 2026-07-31 — inbox/held arrived as fields, not verbs)*
...
| Group | Verb | Notes |
|---|---|---|
| Orient | `get_backlog` | Ready-filtered, priority-ordered, dependency-aware — not a raw dump. *(2026-07-31)* Returns three arrays: `ready`, plus `inbox` and `held` (creation order) so agents orient on captured and paused work without ever being served it. |
...
**Archive is the porcelain word for `task.cancelled`** *(2026-07-31, dogfood steering feedback)*: the plumbing/porcelain split, applied to the status vocabulary. The word mapping: **archive ↔ `task.cancelled`** — the event name, the `cancelled` status value in stored events and API/JSON, and the MCP `update_task` vocabulary are plumbing and never change (T3); everywhere humans read or type, the verb is **archive**. Concretely: the TUI key is `c archive` with confirm copy ending "history stays on the ledger" and an `archived <id>` status message; cancelled tasks render as `archived` wherever a human-facing status appears (one-shot status/backlog tallies, the task-biography status line, edge annotations like `(archived — title)`); and `tuhdoo update --status` documents `archived`, mapping it to the `cancelled` plumbing value (which still passes through for scripts). Rationale: in the TUI, "c cancel" read first as cancelling a UI action, then as deleting the task — but nothing is ever deleted; the event is immutable and full history stays on the data branch. `esc` keeps the word "cancels" for backing out of input entry: that collision on one screen is exactly what this rename resolves.

**The shelves and quick capture** *(2026-07-31, with the D5 status-model revision)*: the dashboard grew two dim sections at the bottom — **ON HOLD above INBOX** (held passed triage, so it sits closer to workable; the display word for the `held` status is **"on hold"** — same porcelain/plumbing split as archive/`cancelled`, decided 2026-07-31: the stored value, API/flag vocabulary, and event bytes stay `held`, humans read "on hold") — with reverse-dim bars and dim rows: parked and captured work is visible without claiming the eye. Shelf rows are ordinary rows: enter opens the detail biography, `c` archives (which is what makes cheap capture reversible), and a task blocked on an inbox/held dependency names that status in its `waiting:` reason through the existing edge annotation (`depends on tuh-xxxx (inbox — title)`). **Quick capture: `i`** opens a single-line input whose enter creates a title-only inbox item as the steering actor — y/n-free by design (capture must cost one keystroke and a title; archive is the undo), armed mode only and fully absent in `--watch` (the key is dead and no bar or footer advertises it). **The TUI deliberately has no promotion key**: promotion is where the tasks-are-prompts bar applies, which means writing a real description — an agent/CLI conversation, not a single-keystroke temptation. One-shot surfaces follow: `tuhdoo backlog` renders On hold and Inbox sections (dim, same order), `status` counts them, `create --status inbox|held` and `update --status open|inbox|held` are the CLI capture and promote/pause/resume paths.
...
Accepted consequences (Cycle 4): the always-open pane is armed unless launched with `--watch` (mitigations: the flag, and archive — `c cancel` until the 2026-07-31 rename above — stays confirm-gated); no runtime arm/disarm toggle — fluidity traded for an ironclad read-only guarantee; `watch` and `top` die as verbs with tombstone errors, not aliases — vestigial names outlive their excuse otherwise.
