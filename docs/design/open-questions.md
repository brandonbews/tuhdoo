...
- **Edge semantics, cycle enforcement, and what a milestone is** *(hit during the B12 cutover, 2026-07-30)*. D5 says "parent edges + dependency edges (a DAG)" without saying whether each relation is a DAG or their union is one; the code chose the union (`opCreateTasks` cycle check), which makes the natural milestone shape — children carry parent edges into it, it depends_on the children — unrepresentable. Three sub-questions: (1) *Semantics:* parents are containment, depends_on is scheduling; should cross-relation "cycles" be legal, with each relation acyclic on its own? (2) *Enforcement honesty:* the check guards only one door — `update_task` does no cycle check, intra-batch edges to pre-existing tasks aren't tracked, and set-union merge (D3) means two acyclic writes can union into a cycle no daemon ever saw. Cycle-freedom structurally cannot be a global invariant here; the core already tolerates dep-cycles by convergent starvation (`core.Ready` returns false forever, identically on every machine). Should write-time checks be per-relation + added to `update_task`, with dep-cycles *surfaced at read time* (status/views warning) instead of pretended away? (3) *Milestones* — **settled 2026-08-03, resolution below.** D5's "just tasks other tasks point into" is underspecified for readiness — a milestone whose deps complete becomes *claimable*, which is meaningless for human-verification work (B12 fenced the v0 milestone with a blocking escalation as a workaround). Do milestones want computed done-ness rather than claimability? The edge-check question is downstream of this one.

**Resolution of (3) — 2026-08-03, milestone grill.** Milestone done-ness is **declared, never computed**, and *no mechanism was added*: "milestone" remains exactly what it already was in the code — a label on a task, with no special casing anywhere (the word appears in no Go file). Computed done-ness was rejected as a trap. The core is pure functions over the event log (T1), so "computed" could only ever mean *done when every `depends_on` task is done* — which fires at precisely the moment verification has **not** happened. That is claimability's failure mode in a nicer hat, and strictly worse: claimable-too-early is annoying, done-too-early is false.

What was broken was the fence, not the done-ness. The two fences carry distinct meanings, and B12 reached for the wrong one because the right one did not exist yet — `held` landed 2026-07-31, the day *after* B12 chose the escalation:

- **`held`** — someone decided this is not to be thought about right now. Quiet shelf; nobody owes an act.
- **blocking escalation** — an attempt stalled and cannot proceed without an answer. Loud inbox. **No attempt, no escalation.** (Now written into `docs/agent-protocol.md`.)

Nothing was typed to humans. A future answerer may not be one — T5 (`relay_answer`: "whatever kind of actor that turns out to be") and T7 (NEEDS INPUT "deliberately avoids naming who answers") already hold that line — so neither an `assignee` field nor a "human work" label was introduced. The v0 milestone was unclaimable not because of *who* had to do it but because its acceptance criterion was too vague for **any** actor to check; the roadmap DoD rewrite of the same date fixes that at the source. Sub-questions (1) and (2) remain open.
