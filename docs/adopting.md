...
There is no server to stand up, no vendor, and no accounts. The entire
shared state is the **data branch**, a git orphan branch (named `tuhdoo`)
inside the repository itself, synced through the remote you already have.
Whoever can write to the repo can plan in it; clone the repo and the plan
comes with it.
...
remote), and prints the MCP snippet for connecting agents. It is
idempotent, so running it again anytime is safe.

Adoption comes with two one-time settings on the git host, both printed by
`init`: exempt the data branch from pull-request and review rules, and
exclude it from CI triggers. Details are in
[the repo-admin section of `joining.md`](joining.md#for-the-repo-admin-branch-protection-and-ci).

## 2. Teammates join

Every other machine, whether a teammate's laptop or your second
workstation, joins with a clone, a binary install, and the same
`tuhdoo init`, which detects the existing data branch on the remote and
adopts it rather than minting a new one. [`joining.md`](joining.md) is the
walkthrough, including clone shapes and identity setup.
...
straight from the binary). It defines the loop they run: claim a task, work
it, escalate questions to a human, and finish with an honest outcome. That
is the agents' half of the contract; your half, writing tasks worth
claiming and steering the queue, is [`steering.md`](steering.md).

## 4. Pick a code workflow

tuhdoo coordinates *who works what*; how code gets written and merged stays
ordinary git, whatever your team already does. An agent fleet still
benefits from a deliberately chosen workflow, and the
...
escalations in place, reprioritize, pause and cancel tasks, and drill into
any task's history. It acts as you. `tuhdoo --watch` is the same screen
...
The work-loop tools (claim, finish, release) are deliberately *not* CLI
...
(a backlog page and per-task pages), so anyone can read the plan from
GitHub or GitLab without installing anything.

The rhythm that follows is small: capture ideas the moment they occur
(`tuhdoo create … --status inbox`), triage the inbox periodically, and keep
escalations answered, because they are the questions your fleet is blocked
on. [`steering.md`](steering.md) covers that loop.
...
reading before you adopt, because it proves adopting costs nothing to undo.
