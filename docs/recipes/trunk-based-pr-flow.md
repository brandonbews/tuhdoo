...
The shape is one task, one branch, and one squash-merged pull request into
a protected default branch. This is the workflow tuhdoo's own repository is
built with (dozens of agent-authored PRs have landed through it) and the
recipe to start from if your team has no strong prior. It is a
recommendation, not protocol: tuhdoo works identically with any git
workflow (see the [recipes overview](README.md) for the boundary).
...
- **Branch protection turns convention into enforcement.** An agent cannot
  be talked out of a rule the host refuses to bypass. With direct pushes
  blocked, a green check required, and squash the only merge method, the
...
1. **Claim the task.** Agents take work through tuhdoo's `claim_next` (or
   `claim_task` for a specific task), never by just starting. See
...
7. **Finish only after the merge lands.** `finish_run(done)` means the
   ...
   unmerged on a branch is not done. See
   [finish honestly](../agent-protocol.md#the-loop) in the agent protocol.
...
- **Auto-delete merged branches**: task branches are disposable by design,
  and deleting them on merge keeps the branch list meaning "work in
  flight".
- **Exempt the tuhdoo data branch** (`tuhdoo` by default, the branch the
  ...
  CI triggers. The daemon pushes it directly, fast-forward only. Details
  are in
...
or rebase merges if your team insists (then `merged_as` reports the rebased
