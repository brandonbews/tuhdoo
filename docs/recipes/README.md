...
fleet look like? Recipes are our answer — recommended patterns for
branching shape, PR conventions, how work lands, and the repo settings that
keep a fleet honest. The boundary is deliberate:

- **Protocol** ships in the tool and in
  [`agent-protocol.md`](../agent-protocol.md) — the claim → work →
  escalate → finish loop, lease semantics, honest outcomes. Agents must
...
- [`trunk-based-pr-flow.md`](trunk-based-pr-flow.md) — one task, one branch,
  one squash-merged PR into a protected default branch. The flow tuhdoo's
  own repository is built with; start here.
