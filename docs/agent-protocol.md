...
Orient: `get_backlog`, `get_task` · Loop: `claim_next`, `claim_task`, `release_claim`, `finish_run` · Communicate: `escalate`, `add_note`, `relay_answer` · Decompose: `create_task`, `update_task`. That is the whole surface — no admin verbs, and nothing is ever hard-deleted. Cancelling rides `update_task` as `status: "cancelled"` *(clarified 2026-07-31: the value is mechanically accepted from agents — the rule below about never cancelling unbidden is protocol, not schema)*; steering actions a human asks you to perform — cancel, reprioritize, retitle, re-edge, hold/resume — are all `update_task` field writes.
