...
This document is for agents working a tuhdoo-managed project; your harness loads it as your instructions. It is your half of the contract; the daemon's half (leases, serialization, sync) is automatic. The ledger — tuhdoo's shared, append-only record of tasks, claims, notes, outcomes, and questions — is agent memory before it is human audit trail: sessions end and contexts compact, and the only continuity between you and your successor is what landed on the ledger. If this document and a `tools/list` response ever disagree, the daemon is right.

The protocol governs how you read and write the ledger. How code gets written and merged is your repo's own workflow, not tuhdoo's. When this document names workflow machinery — pull requests (PRs), merge styles, branch names — that machinery is an example of one way to work, never a tuhdoo requirement. The ledger rules stated around such examples (claiming, confirming, finishing honestly) bind in every workflow.
...
- With no flags, your principal — the identity every event you write is attributed to — is auto-derived at connect. The human half is the local part of `git config user.email`. The daemon mints the agent half from your harness's Model Context Protocol (MCP) `clientInfo.name` plus a per-daemon counter (for example `brandonbews/claude-code-3`, unique per session). If `user.email` is unset or unusable, the shim fails loudly instead of inventing a name.
...
**Leases are session-bound; there is no heartbeat tool.** While your session is connected, the daemon auto-renews every claim you made through it (15-minute time-to-live (TTL), renewed every 5 minutes). If your session dies, renewals stop, the lease lapses within the TTL, the task returns to the ready pool, and your run is auto-closed as `interrupted`. Session death costs auto-renewal, not holdership: reconnect as the same principal before the lease lapses and you can still `add_note`, `finish_run`, or `release_claim` the task you held. Do that first, before claiming anything new.
...
By default `get_backlog` returns exactly three arrays (`ready`, `inbox`, `held`), and the claim tools take from `ready` alone. To orient on anything else, pass the optional `scope` input: section names from `in_progress`, `blocked`, `done`, `cancelled`, `escalations`. Each adds one array of slim rows (id, title, status, priority, labels; no description) plus that section's extras:

- `in_progress`: holder and lease expiry.
- `blocked`: `dep:<task-id>` / `esc:<escalation-id>` waiting reasons.
- `done` / `cancelled`: close stamp and closing actor, newest first.
- `escalations`: full open escalation records in raise order — this is how you find the ID `relay_answer` needs.

Hydration is still `get_task`'s job: discover an ID with scope, then dig.

## The loop

1. **Claim before working.** Call `claim_next` (or `claim_task` if the human named a specific task). Never start work on a task you have not claimed, because another agent may claim it at any moment. `claimed: false` means the ready pool is empty or nothing matches your labels. That is a normal outcome, not an error; report it and stand by rather than retrying in a loop.

   Selection is most urgent first, and **P0 is highest**: priority `0` claims first, larger numbers claim later, and tasks with no priority set come after every prioritized one. Within a rank, order is creation order (task IDs are ULIDs, which sort by creation time). This is the same order as `get_backlog`'s `ready` array. The optional `labels` filter is **all-of**: a task matches only if it carries every requested label; extra labels still match; an unlabelled task never matches a labelled request; and omitting `labels` takes the best ready task outright. Label meanings are your repo's convention; the filter matches strings, nothing more. `claimed: false` doesn't distinguish an empty pool from your filter excluding everything; re-call without `labels` to tell them apart.
...
6. **Finish honestly.** Call `finish_run` with the true outcome and links (branch, PR, commits) plus a summary a human can act on. Exactly four outcomes are reportable:
   ...
   `interrupted` and `superseded` are daemon-synthesized; the agent surface rejects them as reported outcomes. If your repo's workflow rewrites commits as they land (squash or rebase merges, for example), the branch SHAs never reach the default branch; after the merge lands, report the commit(s) on a durable branch that carry the work in `merged_as`, if known. The summary, not a final note, is the handoff; don't write a closing note that duplicates it.
7. **Or release.** If you claimed something you cannot or should not work on, call `release_claim` with a reason. Never sit on a claim you are not working.
...
- **Non-blocking question:** call `escalate`, then keep working on what doesn't depend on it.
...
When a task is too large, decompose it: one batch `create_task` call with the child tasks and the dependency edges between them (use `tmp:<name>` refs within the batch; the whole graph lands atomically or not at all), then point the task you hold at the children with `update_task` `depends_on`. A container is just a task that depends on its children. Work the children through the normal loop, and note why you split it the way you did; decomposition is checkpoint-worthy.
...
- Never write to the tuhdoo data branch (the git orphan branch that stores the ledger) with git directly: all plan writes go through the MCP tools, and the daemon is the sole writer.
