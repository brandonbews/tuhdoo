...
- **Known deferred items:** full-replay-per-write and the daemon's grow-forever event overlay are the flagged optimization point; daemon is unix-only (flock). (The CI workflow file landed 2026-07-30 once the PAT gained the Workflows permission; see CLAUDE.md for the workflow-change review convention.)
