...
- **B9 decisions** (code + comments carry the rationale): HTTP/MCP share one ops layer (`internal/daemon/ops.go`); one `*mcp.Server` per session, principal bound from `X-Tuhdoo-Actor` at initialize (the shim injects it); session liveness = keepalive pings (default 30s, 3 failures tolerated — mirrors T8's three-missed-renewals), renewal every TTL/3; agents cannot write `interrupted`/`superseded` outcomes (daemon-only verdicts); `claim_next` on an empty pool is `claimed:false`, not an error. **For the B11 doc:** the endpoint is stateful, so go-sdk v1.7.0 negotiates protocol ≤2025-11-25 and pings ride the standalone GET stream — a direct streamable client that never opens that GET gets its session closed (~3×keepalive) and its leases lapse; use the shim.
