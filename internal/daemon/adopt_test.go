...
// TestFreshCloneDaemonAdoptsRemoteDataBranch: a plain, genuine clone —
// origin configured, no local data branch (git clone creates no local
// refs/heads/tuhdoo). Its daemon's startup must adopt the remote
// branch, not mint.
...
// shape, then start the clone's daemon and assert adoption: the data
// branch at the remote head, exactly one root, and the seeded task
// visible without a sync cycle.
//
