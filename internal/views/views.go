...
func readme(s *core.State, b buckets) []byte {
	...
	w.WriteString("- [backlog.md](backlog.md) — the work: in progress, ready, and blocked, plus the on-hold and inbox shelves\n")
	w.WriteString("- [escalations.md](escalations.md) — questions from the fleet, awaiting a human\n")
	w.WriteString("- `tasks/` — one page per task: description, status, full history\n\n")
...
func needsHuman(n int) string {
	switch n {
	case 0:
		return "No open questions — the fleet is unblocked.\n\n"
	case 1:
