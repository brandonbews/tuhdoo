...
func readme(s *core.State, b buckets) []byte {
	...
	fmt.Fprintf(&w, "| Held tasks (paused) | %d |\n", len(b.held))
	fmt.Fprintf(&w, "| Inbox (untriaged) | %d |\n", len(b.inbox))
	fmt.Fprintf(&w, "| Done tasks | %d |\n", len(b.done))
	fmt.Fprintf(&w, "| Cancelled tasks | %d |\n", len(b.cancelled))
	fmt.Fprintf(&w, "| Open escalations | %d |\n\n", len(s.OpenEscalations()))

	w.WriteString("- [backlog.md](backlog.md) — ready, in-progress, and blocked work, plus the held and inbox shelves\n")
...
func statusLine(s *core.State, t *core.Task) string {
	...
	case core.StatusDone, core.StatusCancelled:
		return t.Status
	case core.StatusHeld:
		return "held — deliberately paused"
	case core.StatusInbox:
