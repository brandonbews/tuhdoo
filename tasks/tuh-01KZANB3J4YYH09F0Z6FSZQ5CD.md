...
Capture from the 2026-08-05 release session, answering: what is still essential for the first external adopter (Brandon, project #2)? docs/agent-protocol.md is the prose half of the agent contract (claim discipline, blocking-escalation sequence, confirm-before-merge, descriptions-are-prompts) but it lives only in the tuhdoo repo — a foreign repo has no way to hand it to its agents without manually copying a file that then forks silently from canon. The design docs never decided a distribution mechanism (T5 only calls the doc a first-class deliverable).
