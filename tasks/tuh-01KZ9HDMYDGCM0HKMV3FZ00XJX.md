...
The ask: task rows in the TUI list become two-line — full title plus a dim metadata second line.
...
- Line 2: dim metadata indented to gridTitleCol, rendered ONLY when non-empty — a one-line row signals "no labels, no edges" at a glance. Skip-when-empty is deliberate; no placeholder (the task-view labels line needs its `none` for focusability; list rows are not editors).
