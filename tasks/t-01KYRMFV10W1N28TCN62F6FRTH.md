...
The ask: the snapshot event ("full state as of X"), deletion of superseded event files in an ordinary commit, replay starting from the latest snapshot. Four open questions must be settled as design-doc revisions before code (inlined 2026-08-06 from the migrated open-questions capture, tuh-01KZA0VT234XJYVZWT95JM25KW, cancelled as folded-in — the doc it pointed at is tombstoned): (1) triggers — when does compaction run: milestone close, event-count/age threshold, or human command only; (2) initiator — who runs it: a daemon automatic, a CLI verb, or human-only, and what happens when two machines both try (the compaction commit must merge or lose cleanly, never fork state); (3) snapshot verification — how the snapshot is proven equivalent before deletions land: replay-from-snapshot must equal replay-from-full-history byte-identically, checked at write time, not trusted; (4) merge survivability (inlined 2026-08-27 from audit finding tuh-01KZVZT7F8CVJYX1P003BNA3A9, cancelled as folded-in) — the syncer's union merge rule (internal/syncer/merge.go:26-31: a file present on only one side always survives, framed as correctness for append-only events) means merging a compacted head with any pre-compaction head resurrects every deleted event file, so D9's "merges like any other commit" promise fails unless merge learns snapshot awareness; verified 2026-08-27 that nothing snapshot-aware exists anywhere in internal/ or cmd/ — settle at design level before building.
