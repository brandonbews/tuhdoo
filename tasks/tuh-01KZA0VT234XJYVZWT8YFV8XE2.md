...
- Friction signals to watch during the site/ dogfood: does a single priority space serve two unlike projects; do labels keep site vs core work legible (labels are agnostic decoration per D5 — no mechanics without a D5 revision); does claim_next hand a Go-focused agent a CSS task (claim_next has no filters — the claim_next-discovery capture, tuh-01KZA0VT234XJYVZWT8EXV78J5, is the adjacent question and the two grills likely inform each other).
