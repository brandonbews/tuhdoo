...
**Date:** 2026-07-28 (revisions run through 2026-08-05 — noted inline, dated; Cycle 2's were the first, see `002-technology.md`)
...
3. **Host friction is onboarding, not architecture.** CI triggers firing on data-branch pushes, branch-protection rules blocking the daemon — all mitigated by a good `tuhdoo init` and docs. *(Re-affirmed 2026-08-11, adoption-friction grill — after a live Vercel incident: hosted preview builders auto-deploy every branch, so the daemon's per-event pushes produce failing deployments configurable only from the vendor's dashboard. The structural escape — the ledger outside `refs/heads/`, e.g. `refs/tuhdoo/data` — was verified live on GitHub before declining it: a probe push to a custom namespace triggered no webhooks/Actions and passed rulesets untouched, but github.com cannot browse custom refs at all, which would kill D3's browse-the-plan-where-the-repo-lives soul. Decision: the data branch stays a real branch, quirks included; the docs and `init` output call the auto-deploy hazard out plainly. Revisit only on real adopter reports, captured fresh with their evidence — the verified facts live on cancelled task tuh-01KZPW8CZPKF2KTWMA5EAVHQ7F.)*

### D5. Entities: five, DAG-shaped, nothing more *(revised 2026-07-31 — the status model grew inbox and held; revised 2026-08-05 — edge semantics: parents removed, loop posture; notes below)*

| Entity | Role |
|---|---|
| **Task** | Unit of intent: title, description, priority, status, labels, dependency edges (a DAG — agents decompose recursively, not in tidy epic/story/subtask tiers) *(parent edges removed 2026-08-05 — note below)*. Milestones are just tasks other tasks point into. |
...
**Milestones are a label, not a mechanism** *(2026-08-03, milestone grill)*: the table's "milestones are just tasks other tasks point into" is literally true — the word appears in no Go file, and a milestone is simply a task carrying a `milestone` label, with no special casing anywhere. Its done-ness is **declared**, exactly like any other task's; computed done-ness was considered and rejected (the full resolution is preserved in `open-questions.md`'s git history — that file is a tombstone since 2026-08-05). A milestone that should not be worked is parked with `held` — never fenced with a blocking escalation, per the fence rule in `docs/agent-protocol.md`.

**Labels are agnostic decoration** *(2026-08-05, labels grill)*: no label value is ever weight-bearing in the platform. Out of the box tuhdoo knows only that a task can carry labels; taxonomies are the repo's to invent (`milestone` above is this repo's convention, not tuhdoo's) and labels can be ignored entirely. The platform's only mechanical touch is generic matching — `claim_next`'s all-of filter — which treats every value identically. Any future feature that would give a specific label value superpowers requires a revision here first.

**Edges are one relation: `depends_on`** *(2026-08-05, edge grill — PRs #37/#39)*: the `parents` field is removed. It lived display-only from D5's birth to this date — no readiness rule, no view structure, nothing weight-bearing ever hung off it — and the containment it gestured at is expressible in the one relation that *is* weight-bearing: an epic is a container that `depends_on` its children, so it blocks until they land, which is all containment ever meant here. Stored events carrying `parents` stand untouched and ride the unknown-field posture (T3 — additive read, in reverse); newly written `task.created`/`task.updated` events simply no longer carry the key, no version bump. Loop posture, settled the same grill: `update_task` refuses loop-closing edits as **local hygiene, never prevention** — set-union merge (D3) can still union two individually-acyclic writes into a loop no daemon ever saw — so replay marks loop members loudly on every surface ("cyclic — a human must cut an edge"; the core's Blockage answer): detection, with the cut staying human work. Cancelled dependencies keep blocking and are marked ("waiting on cancelled <id>") — cancelled never counts as done, and re-pointing a dependent is a human decision, not a default. Dangling dependency IDs count as met — a deliberate defensive posture: a reference replay cannot resolve degrades to unblocked rather than wedging work forever.
...
**The status model** *(revised 2026-07-31, grill cycle — inbox and held join open/done/cancelled)*: five statuses — `open`, `inbox`, `held`, `done`, `cancelled`. These five are the **stored** vocabulary, complete; the `ready` / `in-progress` / `blocked` a reader meets on display surfaces are derived *situations* of `open`, computed at read time and never stored (the Situation classifier, 2026-08-03; display words in `002` T7). The pressure this relieves: a queue that prices every entry at commission rates (prompt-quality description, priority, scope) forces idea capture into premature quantification, so ideas either don't get captured or get captured badly. The two additions split "not being worked" into its two honest kinds:
...
2. **The confirmation gate: the final verdict is won, not computed** *(added 2026-08-04)*. Before work may be merged or recorded `done`, its claim must be **confirmed**: the daemon examines the remote head it is about to push onto, and only if that state shows no competing confirmation for the task and this claim as the provisional winner does it push a `claim.confirmed` event onto exactly that head. The push is git's atomic compare-and-swap on the branch ref — the remote serializes all comers — and the app-level merge/push path (T2) refuses to carry a second confirmation for a task that already has one, so **at most one confirmation per task can ever land**, by construction, not by probability. Replay's rule is one line: a confirmed claim wins unconditionally. The gate runs in two places (T5): the explicit `confirm_claim` tool, for the moment before an agent merges or arms auto-merge, and **inside every `done`-recording path** (`finish_run` and the HTTP run surface), so even an agent that skips the tool gets refereed — a certified `done` means the referee confirmed the winner, not that an agent believed it finished. With no remote configured, the daemon is the sole writer and confirmation is locally sound and instant (T2: remoteless is a normal state). With a configured remote unreachable, the gate **refuses honestly** with a retryable error — it never guesses (accuracy outranks liveness). *(Two-layer note, 2026-08-05 release grill: "at most one confirmation" and replay's earliest-confirmation rule (`002` T8) are both true, at different layers. The writers' CAS invariant — the remote-serialized push, plus the T2 merge/push path refusing to carry a second confirmation — guarantees at most one confirmation per contest ever lands; that is the guarantee. Replay ADDITIONALLY carries a defensive earliest-confirmation-wins rule: fail-safe determinism for a ledger that violates the writers' invariant — corrupt or hand-crafted — never fail-stop, so every machine converges on the same verdict instead of halting. The referee comments in `internal/core/replay.go` carry the case analysis.)*
3. **Loser handling** *(rewritten 2026-08-04 — the original clause promised a stand-down push and a daemon-known branch name; neither was implementable, and nothing implemented it: the collision harness found the `superseded` shape designed for, guarded for, and written by no one)*. Losers learn at call-time — there is no push channel to a working agent: every claim response carries the warning to confirm before merging, and any tool touched by a provisionally-voided claimant says plainly that it lost. **The daemon is the referee of how attempts ended**: a `finish_run` on a lost attempt is coerced to outcome `superseded`, keeping the agent's reported branch/PR/commits/summary as the salvage record — the branch name is knowable only to the losing agent, so the honest design routes the record through its own report. A loser that never reports leaves a trace anyway: when a voided claim's lease expires unclosed, replay synthesizes a branch-less `superseded` run (the same mechanism as `interrupted`), and the attempt is thereafter closable no longer — one close per attempt, deterministic at every instant. A late-returning loser is told its attempt is closed and pointed at `add_note` for salvage. *(Mechanism note, 2026-08-04 grill: stand-down closes the loser's lease by **overwriting it with a released tombstone** — `{"expires": "<the stand-down instant>", "released": true}` — never by deleting the file. Replay reads a missing lease as lapsed at every instant, so deletion re-adjudicated the contest's past — the promised superseded run degraded to interrupted — and the sync layer's union merge could resurrect the deleted file anyway. The tombstone keeps the lease live before the stand-down and lapsed after it, so the synthesized run dates from the stand-down and settled contest history never moves; `002` T8 carries the never-delete invariant and the merge rule.)*
4. **No claim-time prevention machinery** *(scope narrowed 2026-08-04 — this clause previously read as "no prevention machinery" absolutely; the confirmation gate is a merge-time verdict, not claim-time prevention, and the distinction is deliberate)*: claims never wait, lock, or coordinate; agent-hours are cheap and rare duplicate *attempts* cost less than any claim-time coordination layer. Mitigations against wasted attempts stay: eager push on claim and confirmation events, short fetch intervals, call-time stand-down. What is *not* accepted at any rate is a duplicate **merge** or duplicate certified `done` by protocol-following agents — that is what the gate exists to make impossible rather than improbable.
