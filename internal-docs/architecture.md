...
4. **Replay** (`daemon.refreshLocked` → `store.ReplayInput` → `core.Replay`): memory only (2026-09-10, live replica): the store's in-memory head tree and OID-keyed decode caches supply events and leases, overlay events not yet committed are appended, then a pure fold into `core.State`. Git is touched only by `store.Load` (cold start, reload after an external ref move or a lost compare-and-swap) and `store.Commit`. Claim winner rules and lease expiry are decided inside replay, against event ULID timestamps and the leases map — never a live clock read.
...
7. **Commit** (`store.AppendBatch` → `store.Commit` → `gitx`): each event and rendered file gets a Go-side `BlobOID`; only blobs whose OID differs from the in-memory tree are written (`hash-object -w`), the changed paths go into the private index at `<git-dir>/tuhdoo/index` (`update-index --index-info`, `write-tree`), then `commit-tree` and a compare-and-swap `update-ref` against the in-memory head; a lost CAS reloads head, tree, and index from git and retries, bounded. `Store.Commit` and `Store.FastForward` are the only movers of the ref (the syncer commits merges through them). One batch, one commit, worktree untouched.
...
- Files: `store.go` (`Store`, `Init`, `Load`, in-memory head/tree, `Commit` CAS loop through the private index, `FastForward`, `ReplayInput`, decode caches, `EventsByOID`/`LeasesByOID` for the syncer), `lease.go` (lease format, release tombstones), `batcher.go` (2s debounce coalescing bursts into one commit; views ride along via `AddFiles`).
